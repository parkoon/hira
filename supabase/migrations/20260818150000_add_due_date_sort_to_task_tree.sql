-- 정렬 기준에 목표일을 더한다.
--
--  * 목록이 보여주는 유일한 날짜가 목표일인데 그걸로는 줄을 세울 수 없었다.
--    등록일·수정일은 화면에 없으면서 정렬만 되던 상태라 짝이 맞지 않았다.
--  * 하위작업은 목표일이 비어 있을 수 있다. 오름/내림 어느 쪽이든 빈 건은 뒤로 보낸다 —
--    부호만 뒤집으면 내림차순에서 빈 건이 맨 앞에 몰려 정작 급한 건이 밀린다.
--  * 나머지 정렬 컬럼은 not null이라 nulls last를 붙여도 결과가 달라지지 않는다.
--    한 자리에서 붙여야 기준마다 규칙이 갈리지 않는다.
