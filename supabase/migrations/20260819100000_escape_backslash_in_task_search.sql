-- 검색어의 백슬래시를 이스케이프한다.
--
--  * ilike ... escape '\' 패턴에 넣기 전에 특수문자를 이스케이프하는데,
--    기존 replace(v_q, '\', '\')는 백슬래시를 백슬래시로 바꾸는 no-op였다.
--    백슬래시가 든 검색어는 뒤따르는 문자를 이스케이프해 삼키고(\글 → 글),
--    끝이 백슬래시면 뒤에 붙는 '%'가 리터럴 %가 되어 조용히 어긋난 결과를 냈다.
--  * '\\'로 바꿔 검색어의 백슬래시가 리터럴 백슬래시로 매칭되게 한다.
--    바뀐 것은 이 한 줄이고 함수의 나머지는 20260818150000 그대로다.
