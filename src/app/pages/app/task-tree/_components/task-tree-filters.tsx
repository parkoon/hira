...
import {
  ...
  TASK_TREE_SORT_META,
  TASK_TREE_STATUS_OPTIONS,
} from '../_constants'
import { taskTreeFilterParsers } from '../_utils/task-tree-params'

export function TaskTreeFilters({ assignees }: { assignees: User[] }) {
  const [filters, setFilters] = useQueryStates(taskTreeFilterParsers)

  const filterFields = useMemo<FilterBarField[]>(
    () => [
      { key: 'status', label: '상태', options: TASK_TREE_STATUS_OPTIONS },
      {
        key: 'assignee',
        label: '담당자',
        multiple: false,
      ...
      { key: 'sort', label: '정렬', multiple: false, options: toEnumOptions(TASK_TREE_SORT_META) },
    ],
    [assignees]
  )
  ...
  return (
    <FilterBar
      ...
      value={{
        ...
        sort: toChipValue(filters.sort, TASK_TREE_DEFAULT_SORT),
      }}
      ...
      onChange={(value) =>
        void setFilters({
          ...
          sort: value.sort?.[0] ?? TASK_TREE_DEFAULT_SORT,
          page: 1,
        })
      }
      search={filters.q}
