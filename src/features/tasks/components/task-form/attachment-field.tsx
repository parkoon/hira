...
import {
  ...
  formatFileSize,
  validateAttachment,
} from '@/features/tasks/constants/attachment-policy'
...
/**
 * 고른 파일은 폼이 들고 있는다 — 저장 시 작업과 함께 등록된다 (스펙 §4.2).
...
export function AttachmentField({ existing = [] }: { existing?: Attachment[] }) {
  ...
  const addFiles = (incoming: FileList | null) => {
    ...
    const accepted: File[] = []
    // 용량 상한은 이미 붙어 있는 첨부까지 합쳐 계산한다 (작업당 200MB)
    let totalSize =
    ...
    <div className="space-y-2">
      ...
      >
        ...
        <p className="text-muted-foreground mt-1 text-[11px]">
          파일당 50MB · 작업당 200MB · pdf, docx, xlsx, hwp, 이미지, zip
        </p>
        <input
          ref={inputRef}
          type="file"
