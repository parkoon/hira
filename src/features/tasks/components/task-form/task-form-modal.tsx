...
import {
  ...
  taskFormSchema,
  type TaskFormValues,
} from '@/features/tasks/utils/task-form-schema'
import { Modal } from '@/shared/components/ui/modal'

import { ComplianceStep } from './compliance-step'
import { TaskContentStep } from './task-content-step'
import { TaskMetaAside } from './task-meta-aside'
...
/** 화면 3·4 — 작업 내용 2단계 위저드 (스펙 §4.1). 등록·수정이 같은 폼을 쓴다 */
export function TaskFormModal({
  ...
  const goToComplianceStep = async () => {
    const valid = await form.trigger(TASK_CONTENT_FIELDS)
    if (valid) setStep(2)
  }
