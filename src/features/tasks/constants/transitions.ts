import type { Subtask, SubtaskStatus, TaskStatus } from '@/features/tasks/api/types'

/**
 * 작업(부모) 흐름 — 등록에서 완료까지 한 줄로 이어지는 경로.
 * 부모는 하위작업의 집계라 작업중에서 하위작업 전건 완료 후 바로 완료로 간다.
 * 반려는 요청승인대기중에서 갈라져 나갔다가 재제출로 돌아오고, 회수는 역방향이라
 * 어느 쪽도 이 배열에 넣지 않는다. 워크플로 안내가 두 흐름을 섞으면 읽을 수 없다.
 */
export const TASK_FLOW: TaskStatus[] = ['DRAFT', 'PENDING_APPROVAL', 'IN_PROGRESS', 'DONE']

/** 배포형 기본 흐름 — DBA 검증을 거치지 않는 경우 */
 ...
 * 배포형은 DBA 검증 여부에 따라 9·10단계로 갈리므로 유형만으로는 정해지지 않는다.
 */
