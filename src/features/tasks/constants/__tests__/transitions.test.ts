...
import type { Subtask, TaskStatus } from '@/features/tasks/api/types'
...
describe('TASK_FLOW', () => {
  it('반려를 뺀 모든 작업 상태가 흐름에 빠짐없이 들어간다', () => {
    // 상태를 새로 추가하고 워크플로 안내에 반영하지 않으면 여기서 걸린다
    const covered = [...TASK_FLOW, 'REJECTED' as const].sort()
    const declared = (Object.keys(TASK_STATUS_META) as TaskStatus[]).sort()

    expect(covered).toEqual(declared)
  })

  it('반려는 선형 흐름 밖이라 포함하지 않는다', () => {
    expect(TASK_FLOW).not.toContain('REJECTED')
  })
})
