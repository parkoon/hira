...
export const updateTaskService = async ({
  ...
  draft,
  actorName,
}: UpdateTaskBody): Promise<boolean> => {
  ...
  if (updated.length === 0) {
    throw new AppError('승인을 요청한 작업은 수정할 수 없습니다. 화면을 새로고침해 주세요.')
  }
...
export const getUpdateTaskMutationKey = () => ['/tasks', 'update'] as const

export function useUpdateTaskMutation() {
  const queryClient = useQueryClient()

  return useMutation({
    mutationKey: getUpdateTaskMutationKey(),
